...
import { Heart, TrendingUp, Activity, Calendar, Users } from "lucide-react"
import Link from "next/link"
import { useParentId, useParentChildren } from "@/hooks/api/use-parent"

export default function ParentDashboardPage() {
  const { data: parentId, isLoading: parentIdLoading, error: parentIdError } = useParentId()
  const { data: children = [], isLoading: childrenLoading, error: childrenError } = useParentChildren(parentId || null)

  const loading = parentIdLoading || childrenLoading
  const error = parentIdError || childrenError
  ...
  return (
    <div className="min-h-screen bg-white p-4 md:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        {/* Başlık */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-2">Veli Paneli</h1>
          ...
          <>
            ...
            {children.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <Card>
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">Aktivite Önerileri</CardTitle>
                    <Activity className="h-4 w-4 text-muted-foreground" />
                  </CardHeader>
                  <CardContent>
                    <div className="text-2xl font-bold">
                      {children.reduce(
                        (sum: number, child: any) => sum + (child.activityRecommendations?.length || 0),
                        0
                      )}
                    </div>
                  </CardContent>
                ...
                <Card>
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">Son Değerlendirme</CardTitle>
                    <TrendingUp className="h-4 w-4 text-muted-foreground" />
                  </CardHeader>
                  <CardContent>
                    <div className="text-2xl font-bold">
                      {children.filter((c: any) => c.assessments && c.assessments.length > 0).length}
                    </div>
                    <p className="text-xs text-muted-foreground mt-1">
                      Değerlendirme yapılan çocuk
                    </p>
                  </CardContent>
                </Card>
              </div>
